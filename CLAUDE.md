...
- `docker-compose.yml` -- sets `NODE_FUNCTION_ALLOW_BUILTIN=child_process,fs,path,os`, `N8N_RUNNERS_TASK_TIMEOUT=9000` (150 min in **seconds** — unit is seconds NOT ms; using ms value overflows 32-bit int and crashes the task runner), `N8N_RUNNERS_TASK_REQUEST_TIMEOUT=600` (10 min — match-to-runner timeout, default 60s is too short after long-running tasks), `N8N_RUNNERS_HEARTBEAT_INTERVAL=600` (10 min), `extra_hosts: host.docker.internal:host-gateway` (Docker→Windows ComfyUI access), mounts `comfyui/workflows` as read-only volume, mounts `N8nYtAutomation/videos` → `/home/node/videos` (for `exportLocal` node), `N8N_RESTRICT_FILE_ACCESS_TO` includes `/home/node/videos`, `restart: always`
...
- **Task runner match timeout (different from above)**: `N8N_RUNNERS_TASK_REQUEST_TIMEOUT` controls how long n8n waits to *match* a Code node task to a runner process. Default 60s is too short for this workflow — after a long-running Code node (generateVideo, composeVideo), the runner process may be restarting/reconnecting, and the next Code node fires with the previous runner unavailable. Symptom: `"Task request timed out after 60 seconds. Your Code node task was not matched to a runner within the timeout period. This indicates that the task runner is currently down, or not ready, or at capacity."` Fix: set `N8N_RUNNERS_TASK_REQUEST_TIMEOUT=600` (10 min) in docker-compose.yml and `docker compose up -d` to recreate the container.
...
- **fal.ai queue API — always use returned URLs**: The submit response includes `status_url` and `response_url`. Always use these directly — never reconstruct them from the model name. Manually constructed URLs can differ from what fal.ai actually uses, causing polling to never receive `COMPLETED` and timing out every time even when clips finish successfully.
- **FFmpeg `-shortest` cuts audio**: `-shortest` stops encoding at the shorter stream. Frame quantization at `fps=30` makes the video a fraction shorter than `audioDuration` — causing the last 1–2 words of the voiceover to be cut off. Fix: build the video 1s longer than audio (add buffer to `targetClipDur`), then use `-t audioDuration` to trim the output to the exact voice length.
...
| 12 | Generate Background Music (fal.ai Stable Audio) | code | `generateMusic` | fal.ai Stable Audio: 45s WAV→MP3, 300s timeout, fallback silence if falApiKey not set |
...
- **v8.10** (2026-04-16): Gemini prompt rewrite — root-cause fix for mid-sentence scripts. `scriptGen` systemPrompt fully rewritten. Actual root cause of v8.8 failure was ambiguous wording: "Kicker flows seamlessly back to the Hook" made Gemini end with conjunctions/prepositions (`"...realizing that"`, `"...rewritten before"`) as syntactic cliffhangers. New rule: "The replay loop is THEMATIC, not grammatical. The Kicker must stand alone as a finished thought." Added hard rules (end with `.`/`!`/`?`, never end in conjunction/preposition/article/comma), soft target (75-95 words), concrete bad/good examples, final self-check instruction. v8.9's regex validation + retry + truncation kept as defense-in-depth.
- **v8.9** (2026-04-16): Permanent script mid-sentence fix (defense-in-depth). v8.8's prompt-only fix failed — Gemini still produced 87-word scripts ending with `"...realizing that"`. Three-layer fix: (1) `scriptGen` adds post-Gemini regex check `/[.!?]["')\]]?\s*$/` — throws `"Script ends mid-sentence..."` if not, n8n auto-retries up to 3x (`retryOnFail: true, maxTries: 3`); (2) new prompt rule "SCRIPT field MUST end with a period, question mark, or exclamation mark — never a conjunction, preposition, article, or comma"; (3) `parseScript` safety-net truncation — if all retries still fail, truncates to last complete sentence via `matchAll(/[.!?]["')\]]?(?=\s|$)/g)`. Throws only if truncated result <65 words.
- **v8.8** (2026-04-15): Script mid-sentence cutoff fix (prompt-only, insufficient). `scriptGen`: word count rule changed to "always finish the final sentence even if slightly over 90 words, never stop mid-sentence"; Kicker rule updated to require a complete sentence. Root cause: Gemini was treating the 90-word limit as a hard stop, cutting off mid-sentence. Superseded by v8.9.
- **v8.7** (2026-04-14): Voiceover cutoff fix. `composeVideo`: `-shortest` → `-t audioDuration`, added 1s buffer to `targetClipDur`. Fixes last 1–2 words being cut off due to frame quantization making video fractionally shorter than audio.
- **v8.6** (2026-04-14): Kling reliability overhaul. `generateVideo`: all 6 clips submitted + polled in parallel (`Promise.allSettled`); uses `status_url`/`response_url` from fal.ai response directly (manual URL construction was root cause of all timeouts — polling hit wrong endpoint, never saw COMPLETED); 1 auto-retry per clip; proceeds with ≥4/6 clips; per-clip timeout 3600s.
- **v8.5** (2026-04-14): Kling timeout fix. `generateVideo`: per-clip poll timeout 1200s→1800s, exec timeout 7800s→9000s (matches `N8N_RUNNERS_TASK_TIMEOUT`). Fixes "Kling timeout after 1200s" on fal.ai peak load.
