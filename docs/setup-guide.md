...
The workflow uses a 4-provider fallback stack for image generation. All providers are free. Keys are passed as environment variables (not n8n credentials).

### Provider 1: Together.ai (Primary — Best Quality)

**Used for**: AI-generated images via FLUX.1-schnell-Free model (~2 seconds per image)

1. Sign up at [together.ai](https://www.together.ai/)
2. Go to **Settings** → **API Keys**
3. Copy your API key
4. Add to `.env` file: `TOGETHER_API_KEY=your-key-here`

**Free tier**: Unlimited FLUX.1-schnell-Free requests for 3 months + $25 credit for other models.

### Provider 2: Pollinations.ai (Secondary — No Rate Limits)

**Used for**: AI-generated images via FLUX model (10-20 seconds per image)
...
### Provider 4: FFmpeg Gradient (Always Available)

**Used for**: Solid-color gradient backgrounds as a last resort. No API key needed — uses local FFmpeg.

### How It Works

The Generate Images node tries providers in order: Together.ai → Pollinations.ai → Pexels → FFmpeg gradient. If a provider's key is missing or the request fails, it automatically tries the next one. The workflow **never fails** on image generation.

### Apply Keys

After adding keys to your `.env` file, restart the container:
```bash
cd /path/to/n8n
docker compose up -d
```

The `docker-compose.yml` reads these environment variables and passes them to the n8n container.
...
- [ ] 8 vertical images were generated by Together.ai / Pollinations.ai / Pexels (768x1344, base64)
...
| 2 | Together.ai | Env var `TOGETHER_API_KEY` | Generate Images | $0.00 (free tier) |
| 3 | Pollinations.ai | Env var `POLLINATIONS_SECRET_KEY` | Generate Images | $0.00 (free) |
| 4 | Pexels | Env var `PEXELS_API_KEY` | Generate Images | $0.00 (free) |
| 5 | YouTube OAuth2 | OAuth2 | Upload to YouTube, Add to Playlist | Free |
...
- [ ] Set up image generation API keys in `.env` file (`TOGETHER_API_KEY`, `POLLINATIONS_SECRET_KEY`, `PEXELS_API_KEY`)
